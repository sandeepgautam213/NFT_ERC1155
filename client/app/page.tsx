...
export default function Home() {
  ...
  const mintNFT = async () => {
    ...
    try {
      ...
      const mint: ethers.ContractTransactionResponse | undefined =
        await contract?.mint(0, 1, options);
      await mint?.wait();
  ...
  const burn = async () => {
    try {
      ...
      localStorage.setItem(
        "burnPrice",
        parseInt(supply.toString()) === 0 ? "0.0001" : burnPrice.toString()
      );
      ...
      <div>
        <nav className="flex justify-between items-center bg-blue-500 p-4">
          ...
          {connect ? (
            ...
            <button
              className="bg-white text-blue-500 px-4 py-2 rounded-md shadow-md hover:bg-blue-400 hover:text-white"
